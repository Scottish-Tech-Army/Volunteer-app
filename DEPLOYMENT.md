...
2. Add changelog notes - a quick summary (a line or two will usually do) of what this new version does (NB the public will see these notes in the Play Store / App Store, so keep them easy to understand and relevant to app users). These should be added to the 'Unreleased' section of [app/android/app/CHANGELOG.md](app/android/app/CHANGELOG.md). You may wish to use the structure suggested by the [Keep a Changelog](https://keepachangelog.com/) project, and use subheadings to indicate additions, changes, fixes, etc.
