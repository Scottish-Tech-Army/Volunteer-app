...
describe('Test the videos service', () => {
  test('getVideoWebpagePlayerOnly gets Vimeo URL', async () => {
    ...
    const fakeVideoWebPage = `https://vimeo.com/${faker.number.int({
      min: 100000000,
      max: 999999999,
    })}`
    ...
    const youTubeGetVideoWebpagePlayerOnlySpy = jest.spyOn(
      youTubeService,
      'getVideoWebpagePlayerOnly',
    )

    // Run the function we're testing
    const response = await videosService.getVideoWebpagePlayerOnly(
      fakeVideoWebPage,
    )

    // Check our test expectations are met
    expect(vimeoGetVideoWebpagePlayerOnlySpy).toHaveBeenCalledTimes(1)
    expect(vimeoGetVideoWebpagePlayerOnlySpy).toHaveBeenCalledWith(
      fakeVideoWebPage,
    )
  ...
  test('getVideoWebpagePlayerOnly gets YouTube URL', async () => {
    // Set up fake test data
    const fakeVideoWebPage = `https://youtube.com/watch?v=${faker.lorem.word(
      11,
    )}`
    const fakeVideoWebPagePlayerOnly = faker.internet.url()
    ...
    expect(youTubeGetVideoWebpagePlayerOnlySpy).toHaveBeenCalledWith(
      fakeVideoWebPage,
    )
    expect(response).toEqual(fakeVideoWebPagePlayerOnly)
