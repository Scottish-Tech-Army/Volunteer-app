...
function combineProjectsAndResources(projects, resources) {
  return resources.map(resource => {
    const project = projects.filter(
      project => project.it_key === resource.it_key,
    )[0]
...
function formatProjectResourceFromAirTable(projectResource) {
  ...
  for (const [fieldName, fieldProperties] of Object.entries(
    projectResourceFieldDefinitions,
  )) {
    ...
    switch (fieldName) {
      case 'required':
        projectResourceFormatted[fieldName] =
          projectResource[fieldName] === 1
            ? '1 person'
            : `${projectResource[fieldName]} people`
        break
      case 'skills':
